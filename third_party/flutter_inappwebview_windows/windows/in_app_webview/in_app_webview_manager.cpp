    ...
    ChannelDelegate(plugin->registrar->messenger(), InAppWebViewManager::METHOD_CHANNEL_NAME)
  {
    ...
    if (!rohelper_) {
      ...
      if (rohelper_->WinRtAvailable()) {
        ...
        NativeLog("GraphicsCaptureSession supported");
        ...
        NativeLog(std::string("GraphicsContext valid=") + (valid_ ? "true" : "false"));
      } else {
        NativeLog("RoHelper WinRtAvailable=false");
      }
    }
    ...
    NativeLog("InAppWebViewManager ctor end");
  }
