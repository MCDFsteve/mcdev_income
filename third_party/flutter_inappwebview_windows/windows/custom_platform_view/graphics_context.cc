...
namespace flutter_inappwebview_plugin
{
  ...
  {
    NativeLog("GraphicsContext ctor start");
    device_ = CreateD3DDevice();
    if (!device_) {
      NativeLog("GraphicsContext CreateD3DDevice failed");
      return;
    }
    ...
    if (FAILED(CreateDirect3D11DeviceFromDXGIDevice(
      ...
      NativeLog("GraphicsContext CreateDirect3D11DeviceFromDXGIDevice failed");
      return;
    }

    valid_ = true;
    NativeLog("GraphicsContext ctor end (valid=true)");
  }
